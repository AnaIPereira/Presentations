...
\section{Parallel scaling at higher thread count}
\begin{frame}
\begin{figure}
\includegraphics[scale=.25]{images/plotparallel.png}
\end{figure}
\end{frame}
...
\item  $N$ \textcolor{orange}{workers} $\longrightarrow$ $N-2$ \textcolor{blue}{SortBlocks} are created; 
\item \textcolor{blue}{SortBlocks} routine starts; 
\end{itemize}
\begin{figure}
...
\textbf{division of terms} of \textcolor{orange}{workers} to \textcolor{blue}{SortBlocks}; inverse tree at the end with the
...
\item If a \textbf{term} does not fit at the end of a block: Writter fills the \textcolor{blue}{SortBlocks} all the way up to the end of the block, \textbf{overlapping} to the next;
...
\end{figure}
\begin{itemize}
\item Require fewer SortBlocks - master buffers not expanded so often;
...
\end{figure}
\begin{itemize}
\item Writter (W) detects if there is a Reader (R) waiting for it;
...
\item Performance could be increased by having one last stage of sorting of tha last 2 \textcolor{blue}{SortBlocks} and \textcolor{magenta}{Master} only writting;
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Add a new SortBot at the end of the merging tree to do the sort of the streams that comes from last 2 SortBlocks;
...
\item  \textcolor{blue}{SortBot} changes - $20\%$ improvement for some tests;
\item Current working on improving  \textcolor{blue}{SortBot} parallelization by addition of extra SortBot at the end of the tree.
\end{itemize}
\end{frame}
