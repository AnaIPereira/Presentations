...
\item $K_{l3}$, $0^+ \longrightarrow 0^+$, neutron decay (SL) and $K_{l2}$ (L);
...
\begin{frame}{ $V_{us}$ from $K_{l3}$ - Semi-leptonic kaon decays}
\textbf{$K_{\ell3}$ decays:}
\[
K \to \pi \ell \nu_\ell,
\qquad \ell=e,\mu,
\]
mediated by the weak transition
\[
s \to u\,\ell\,\bar{\nu}_\ell.
\]
\begin{equation*}
...
\vspace{0.2cm}
\scriptsize
Lattice results (squares and empty circles) for $f_+(0)$ and ChPT estimates (filled blue circles). The black squares and grey bands indicate averages.
\end{column}
...
\begin{frame}{Lattice input for $f_K/f_\pi$}
...
\vspace{0.2cm}
\scriptsize
Lattice results for $f_K/f_\pi$ in pure QCD including
isospin-breaking corrections. Black squares and grey bands indicate averages.
\end{column}
