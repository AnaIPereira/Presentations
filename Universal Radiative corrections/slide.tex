...
\section{$V_{ud}$ from pion $\beta$ decay}

\begin{frame}{$V_{ud}$ from pion $\beta$ decay}
...
\begin{itemize}
    ...
    \item $\Delta_{RC}^{\pi\ell}$ electroweak radiative correction (short distance and long distance)
\end{itemize}
...
\begin{itemize}
    \item NLL matching of LEFT and ChPT
    %\item Uses lattice-QCD input for the $\gamma W$ box
    \item Theory uncertainty reduced by a factor $\sim 3$ (preparing theory for PIONEER)
\end{itemize}
